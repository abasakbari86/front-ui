...
log_folder="${FRONTUI_LOG_FOLDER:=/var/log/front-ui}"
mkdir -p "${log_folder}"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/abasakbari86/front-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all front-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/abasakbari86/front-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/front-ui https://raw.githubusercontent.com/abasakbari86/front-ui/main/front-ui.sh
    chmod +x ${frontui_folder}/front-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/abasakbari86/front-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del front-ui
        rm /etc/init.d/front-ui -f
    else
        systemctl stop front-ui
        systemctl disable front-ui
        rm ${frontui_service}/front-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/abasakbari86/front-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${frontui_folder}/front-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${frontui_folder}/front-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the FRONT-UI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${frontui_folder}/front-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${frontui_folder}/front-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${frontui_folder}/front-ui setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${frontui_folder}/front-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "front-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "front-ui and xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service front-ui restart
    else
        systemctl restart front-ui
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "front-ui and xray Restarted successfully"
    else
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service front-ui status
    else
        systemctl status front-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add front-ui default
    else
        systemctl enable front-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "front-ui Set to boot automatically on startup successfully"
    else
        LOGE "front-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "front-ui Autostart Cancelled successfully"
    else
        LOGE "front-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'front-ui[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u front-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-front-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-front-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-front-ui.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-front-ui.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/front-ui -z /usr/bin/front-ui https://github.com/abasakbari86/front-ui/raw/main/front-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service front-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status front-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'front-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled front-ui)
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        curl -fLRo ${frontui_folder}/bin/${dat}.dat -z ${frontui_folder}/bin/${dat}.dat \
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${frontui_folder}/front-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart front-ui 2>/dev/null || rc-service front-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="front-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}front-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; front-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; front-ui restart"
                reloadCmd="systemctl reload nginx ; front-ui restart"
                ;;
            2)
                LOGD "It's recommended to put front-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; front-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${frontui_folder}/front-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${frontui_folder}/front-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${frontui_folder}/front-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${frontui_folder}/front-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${frontui_folder}/front-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${frontui_folder}/front-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${frontui_folder}/front-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
|  ${blue}front-ui restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}front-ui update-all-geofiles${plain}   - Update all geo files             │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}FRONT-UI Panel Management Script${plain}                │
