...
// GetUpdateInfo checks GitHub for the latest front-ui release.
...
func (s *PanelService) StartUpdate() error {
	...
	updateScript := fmt.Sprintf("set -o pipefail; %s -fLs https://raw.githubusercontent.com/abasakbari86/front-ui/main/update.sh | %s", shellQuote(curl), shellQuote(bash))

	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("front-ui-web-update-%d", time.Now().Unix())
		cmd := exec.Command(systemdRun,
			"--unit", unitName,
			"--setenv", "FRONTUI_MAIN_FOLDER="+mainFolder,
			"--setenv", "FRONTUI_SERVICE="+serviceFolder,
	...
	cmd.Env = append(os.Environ(),
		"FRONTUI_MAIN_FOLDER="+mainFolder,
		"FRONTUI_SERVICE="+serviceFolder,
	)
...
func fetchLatestPanelVersion() (string, error) {
	...
	resp, err := client.Get("https://api.github.com/repos/abasakbari86/front-ui/releases/latest")
	if err != nil {
		return "", err
	}
...
func resolveUpdateFolders() (string, string) {
	mainFolder := os.Getenv("FRONTUI_MAIN_FOLDER")
	if mainFolder == "" {
	...
	if mainFolder == "" {
		mainFolder = "/usr/local/front-ui"
	}

	serviceFolder := os.Getenv("FRONTUI_SERVICE")
	if serviceFolder == "" {
