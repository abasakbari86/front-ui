...
import (
	...
	"github.com/abasakbari86/front-ui/v2/database/model"
	frontuilogger "github.com/abasakbari86/front-ui/v2/logger"
	"github.com/op/go-logging"
)

// front-ui logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	loggerInitOnce.Do(func() {
		frontuilogger.InitLogger(logging.ERROR)
	})

	dbDir := t.TempDir()
	logDir := t.TempDir()

	t.Setenv("FRONTUI_DB_FOLDER", dbDir)
	t.Setenv("FRONTUI_LOG_FOLDER", logDir)
	...
	if err := database.InitDB(filepath.Join(dbDir, "front-ui.db")); err != nil {
...
func readIpLimitLogPath() string {
	folder := os.Getenv("FRONTUI_LOG_FOLDER")
	if folder == "" {
