# FRONT-UI Development Guide

## Project Overview

FRONT-UI is a web-based control panel for managing Xray-core servers. It's a Go application using Gin web framework with embedded static assets and SQLite database. The panel manages VPN/proxy inbounds, monitors traffic, and provides Telegram bot integration.
...
```bash
# Build (creates bin/front-ui.exe)
go run tasks.json → "go: build" task

# Run with debug logging
FRONTUI_DEBUG=true go run ./main.go
# Or use task: "go: run"
...
- DB path: Configured via `config.GetDBPath()`, typically `/etc/front-ui/front-ui.db`
...
- Environment vars: `FRONTUI_DEBUG`, `FRONTUI_LOG_LEVEL`, `FRONTUI_MAIN_FOLDER`
...
Both `install.sh` and `front-ui.sh` follow these patterns:
...
- Binary: `/usr/local/front-ui/`
- Database: `/etc/front-ui/front-ui.db`
- Logs: `/var/log/front-ui/`
- Service: `/etc/systemd/system/front-ui.service.*`

## Testing & Debugging

- Set `FRONTUI_DEBUG=true` for detailed logging
- Check Xray process: `front-ui.sh` script provides menu for status/logs
- Database inspection: Direct SQLite access to front-ui.db
